...
export default function CheckWeatherPage() {
  ...
  async function loadWeather(lat: number, lon: number, label: string) {
    ...
    try {
      const response = await fetch("/api/weather", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lat, lon, days: 7 }) });
  ...
  return <main className="min-h-screen bg-[#f6f8f4] text-[#14231A]">
    ...
    <section className="mx-auto max-w-6xl px-5 py-12 sm:px-8 md:py-16">
      ...
      {weather?.daily?.length ? <section className="mt-5 rounded-[28px] border border-[#dce6dc] bg-white p-5 sm:p-7"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#16A34A]">Seven-day outlook</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">{weather.daily.map((day) => <div key={day.dt} className="rounded-2xl bg-[#f5f8f4] p-3"><p className="text-xs font-bold text-[#526558]">{day.dt ? new Date(day.dt * 1000).toLocaleDateString(undefined, { weekday: "short" }) : "Day"}</p><p className="mt-3 text-sm font-bold capitalize text-[#173c28]">{day.weather?.[0]?.description ?? "Conditions"}</p><p className="mt-2 text-sm text-[#526558]">{Math.round(day.temp?.max ?? 0)}° / {Math.round(day.temp?.min ?? 0)}°</p></div>)}</div></section> : null}
