...
export default function DashboardOverviewPage() {
  ...
  return <div className="space-y-5 md:space-y-7">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.3fr_.7fr]">
      ...
      <div className="farm-card p-5 md:p-6 xl:col-span-2"><div className="flex items-start justify-between gap-3"><div><p className="eyebrow">Advice for your farm</p><h2 className="mt-2 text-xl font-bold tracking-[-.03em] text-[#183127]">What to do next</h2></div><CloudRain className="h-5 w-5 text-[#0b8f45]" /></div>{latestAdvice ? <><p className="mt-5 text-base font-bold leading-6 text-[#183127]">{latestAdvice.title}</p><p className="mt-2 whitespace-pre-line text-sm leading-7 text-[#617067]">{latestAdvice.text}</p><p className="mt-4 text-xs text-[#829087]">Updated {dateLabel(latestAdvice.createdAt)}</p></> : <p className="mt-5 text-sm leading-6 text-[#617067]">Get a fresh, practical plan for today&apos;s farm work whenever you are ready.</p>}<button onClick={() => void refreshFarmAdvice()} disabled={!canGetAdvice} className="action-primary mt-5 disabled:cursor-not-allowed disabled:opacity-50">{adviceLoading ? "Preparing advice…" : "Get new advice"}</button>{!subscriptionActive ? <p className="mt-3 text-sm text-[#a2423b]">Renew your subscription to receive fresh farm advice.</p> : !hasFarmArea ? <p className="mt-3 text-sm text-[#617067]">Add your state and local government area in Settings to get tailored advice.</p> : !weather ? <p className="mt-3 text-sm text-[#617067]">We are preparing your local weather before advice can be generated.</p> : !crops.length ? <p className="mt-3 text-sm text-[#617067]">Add at least one crop before requesting farm advice.</p> : null}</div>
    </section>
  </div>;
}
