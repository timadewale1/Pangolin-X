...
import { getCropGrowthInfo, getStageDayRange } from "@/lib/cropGrowth";
import { normalizeSoilSummary, translateSoil } from "@/lib/soil";
import type { WeatherData } from "@/lib/dashboard-types";
import Loader from "@/components/Loader";
import { addCropAdvisory, fetchLatestCropAdvisory } from "@/lib/firestore";

export default function CropDetailPage() {
  const params = useParams<{ cropId: string }>();
  const cropId = params.cropId;
  const { farm, saveCropStatus, user } = useDashboard();
  ...
  const stageRange = useMemo(() => getStageDayRange(cropId, stage), [cropId, stage]);
  ...
  useEffect(() => {
    if (!user) return;
    fetchLatestCropAdvisory(user.uid, cropId).then((saved) => {
  ...
  const saveProgress = async () => {
    ...
    if (!Number.isFinite(days) || days < stageRange.min || days > stageRange.max) return;
        ...
        <div className="grid gap-0 lg:grid-cols-[1.1fr_1fr]">
          ...
          <div className="p-5 md:p-6">
            ...
            <div className="mt-5 rounded-xl border border-[#b9ddc3] bg-[#f8fcf8] p-4">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm font-semibold text-[#34473a]">Crop stage<select value={stage} onChange={(event) => { const nextStage = event.target.value; setStage(nextStage); const range = getStageDayRange(cropId, nextStage); const currentDays = Number(daysPlanted); if (!Number.isFinite(currentDays) || currentDays < range.min || currentDays > range.max) setDaysPlanted(String(range.min)); }} className="mt-2 w-full rounded-xl border border-[#d6e1d6] bg-white px-3 py-2.5 font-medium outline-none focus:border-[#16A34A]"><option value="just_planted">Just planted</option><option value="vegetative">Vegetative</option><option value="flowering">Flowering</option><option value="maturing">Maturing</option><option value="harvest_ready">Harvest ready</option></select></label><label className="text-sm font-semibold text-[#34473a]">Days planted<span className="ml-1 text-xs font-medium text-[#617067]">({stageRange.min}–{stageRange.max} days)</span><input value={daysPlanted} onChange={(event) => setDaysPlanted(event.target.value)} min={stageRange.min} max={stageRange.max} type="number" inputMode="numeric" className="mt-2 w-full rounded-xl border border-[#d6e1d6] bg-white px-3 py-2.5 font-medium outline-none focus:border-[#16A34A]" /></label></div>
              <button type="button" onClick={() => void saveProgress()} disabled={savingProgress || !daysPlanted || Number(daysPlanted) < stageRange.min || Number(daysPlanted) > stageRange.max} className="action-primary mt-4 inline-flex items-center gap-2 disabled:opacity-60"><Save className="h-4 w-4" />{savingProgress ? "Saving…" : "Save crop progress"}</button>
            </div>
          </div>
        </div>
      </section>
