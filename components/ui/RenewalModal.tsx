...
export default function RenewalModal({ open, onClose, currentPlan = null, email = null, onRenewed }: Props) {
  ...
  const downloadPDFReceipt = () => {
    ...
    doc.text(`Original Amount: ₦${Math.round((receipt.amount ?? 0) / 100).toLocaleString()}`, 20, 55);
    if (receipt.discount && receipt.discount > 0) {
      doc.text(`Pro-rate Discount: ₦${Math.round((receipt.discount ?? 0) / 100).toLocaleString()}`, 20, 65);
    }
    doc.text(`Final Charge: ₦${Math.round((receipt.finalCharge ?? receipt.amount ?? 0) / 100).toLocaleString()}`, 20, 75);
    doc.text(`Date: ${receipt.date ? new Date(receipt.date).toLocaleString() : ''}`, 20, 85);
  ...
  const [receipt, setReceipt] = useState<{ reference: string; amount: number; plan: string; date: string; discount?: number; finalCharge?: number } | null>(null);
  ...
  async function startPayment() {
    ...
    try {
      ...
      if (!initRes.ok) throw new Error(initJson?.message || 'Failed to initialize payment');
        ...
        callback: typeof window !== 'undefined' ? async function(response: { reference?: string }) {
          try {
            ...
            if (vr.ok && vdata.success) {
              ...
              setReceipt({ reference: refOut, amount: amountN, plan: selected, date: new Date().toISOString(), discount, finalCharge });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-black/40" onClick={() => { if (!loading && !verifying) onClose(); }} />
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg relative p-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { if (!loading && !verifying) onClose(); }} className={`text-gray-500 ${loading || verifying ? 'opacity-50 pointer-events-none' : ''}`}>Close</button>
        </div>
        ...
        {!receipt ? (
          <>
            <div className="mt-4">
              ...
              <div className="font-medium text-green-700 mt-1">{currentPlan ? currentPlan.toUpperCase() : 'None'}</div>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-2 mt-2">
                <button
                  onClick={() => {
                    if (selected !== 'monthly' && !confirm('Change plan to Monthly? You will be charged the full price.')) return;
                    setSelected('monthly');
                  }}
                  className={`px-3 py-2 rounded border ${selected === 'monthly' ? 'bg-green-50 border-green-600' : ''}`}>
                  Monthly — ₦1,500
                </button>
                <button
                  onClick={() => {
                    if (selected !== 'yearly' && !confirm('Change plan to Yearly? You will be charged the full price.')) return;
                    setSelected('yearly');
                  }}
                  className={`px-3 py-2 rounded border ${selected === 'yearly' ? 'bg-green-50 border-green-600' : ''}`}>
                  Yearly — ₦15,000
                </button>
              </div>
            </div>
          ...
          <div className="mt-4 bg-green-50 p-4 rounded">
            ...
            <div className="mt-2 font-medium text-green-800">Reference: {receipt.reference}</div>
            <div className="text-sm text-gray-700">Plan: {receipt.plan}</div>
            <div className="text-sm text-gray-700">Original Amount: ₦{Math.round((receipt.amount ?? 0) / 100).toLocaleString()}</div>
            {receipt.discount && receipt.discount > 0 && (
              <div className="text-sm text-gray-700">Pro-rate Discount: ₦{Math.round(receipt.discount / 100).toLocaleString()}</div>
            )}
            <div className="text-sm text-gray-700 font-bold">Final Charge: ₦{Math.round((receipt.finalCharge ?? receipt.amount ?? 0) / 100).toLocaleString()}</div>
            <div className="text-sm text-gray-700">Date: {new Date(receipt.date).toLocaleString()}</div>
          </div>
        )}
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          {!receipt ? (
            <>
              <button onClick={() => { setSelected(currentPlan ?? 'monthly'); }} className="px-3 py-2 rounded border">Cancel</button>
              <button onClick={startPayment} disabled={loading || verifying} className="px-4 py-2 bg-green-600 text-white rounded font-semibold">
                {loading || verifying ? <span className="flex items-center gap-2"><Loader /> Processing</span> : `Pay & Renew`}
              </button>
            </>
          ) : (
            <>
              <button onClick={downloadPDFReceipt} className="px-3 py-2 rounded border">Download PDF</button>
              <button onClick={() => { setReceipt(null); }} className="px-3 py-2 rounded border">Close receipt</button>
              <button onClick={() => { setReceipt(null); onClose(); }} className="px-4 py-2 bg-green-600 text-white rounded font-semibold">Done</button>
            </>
          )}
        </div>
      </div>
